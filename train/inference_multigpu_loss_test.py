...
# python inference_multigpu_loss_test.py --id TEST --path-data "data/uniprot_sprot.csv" --path-offsd "data/uniprot_sprot_offset_dict.json" --save-interval-step 2 --path-weights "results/run24/model/2021-05-08_13:35:03_step00140000.pt"
...
def inferer(rank, world_size):
    ...
    if args.rank == 0:
        ...
        ds = CLASPDataset(path="/home/mmp/hdd1/ProTexCLIP/uniprot_sprot.csv",
                  text_sampler=text_sampler,
                  bioseq_sampler=bioseq_sampler,
                  text_tok=text_tok,
                  bioseq_tok=bioseq_tok)
        print(f"{datetime.now()} rank: {args.rank} dataset setup")

        bs = 16*8
        dl = DataLoader(ds, bs, True, num_workers=1, drop_last=True)
        dl_iter = iter(dl)
        print(f"{datetime.now()} rank: {args.rank} dataloader iter setup")
        ...
        print(f"{datetime.now()} rank: {args.rank} got one batch")
        ...
        sim_text   = (einsum('i d, j d -> i j', text_latents, bioseq_latents) * temp)
        sim_bioseq = (einsum('i d, j d -> i j', bioseq_latents, text_latents) * temp)

        labels = torch.arange(text_latents.shape[0])

        loss = ((F.cross_entropy(sim_text, labels) + F.cross_entropy(sim_bioseq, labels)) / 2).mean()

        print(f"{datetime.now()} rank: {args.rank} loss: {loss.item():.6f} (random init loss: {torch.tensor(text_latents.shape[0]).log().item():.6f})")
